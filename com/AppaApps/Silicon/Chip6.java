   ...
   {final int               seq = nextGateNumber();                             // Sequence number for this gate
    ...
    class DrivePin implements Comparable<DrivePin>                              // Shows that this gate drives another gate either on a specific pin or on any pin if the gate is commutative
     {final String drives;                                                      // Drives this named gate
      final Boolean pin;                                                        // Null any pin, true - input pin 1, false - input pin 2
      DrivePin(String Drives, Boolean Pin)                                      // Construct a pin drive specification targeting a specified input pin
       {drives = Drives; pin = Pin;
       }
      DrivePin(String Drives)                                                   // Construct a pin drive specification targeting any available input pin
       {drives = Drives; pin = null;
       }
      Gate gate() {return getGate(drives);}                                     // Details of gate being driven
      public int compareTo(DrivePin a)                                          // So we can add and remove entries from the set of driving gates
       {return drives.compareTo(a.drives);
       }
    ...
    public Gate(Operator Op, String Name, String Input1, String Input2)         // User created gate with a user supplied name and inputs
     {name = validateName(Name);
      op = Op;
      gates.put(name, this);
      if (commutative(op))                                                      // Any pin will do
       {if (Input1 != null) impinge(Input1);
        if (Input2 != null) impinge(Input2);
       }
      else                                                                      // Pin order is important
    ...
    boolean whichPinDrivesPin(String driving)                                   // True if the driving gate is using pin 1 to drive this gate, else false meaning it is using pin 2
     {final Gate d = getGate(driving);                                          // Driving gate
      ...
      if (d.drives.size() == 0) {say("Gate name:", driving, "is referenced as driving gate:", name, "but does not drive it"); return false;}
    ...
    void compileGate()                                                          // Locate inputs for gate so we do not have to look them up every time
     {final int N = drives.size();
      if (N == 0)
       {if (op == Operator.Output) return;                                      // An output gate does not drive any other gate
        stop("Gate", name, "does not drive any other gate");
        return;
       }
      if (N > 2) stop("Gate", name, "drives", N, "gates, but a gate can drive no more than 2 other gates");
      for (DrivePin t : drives)                                                 // Connect targeted gates back to this gate
       {final Gate T = t.gate();
        if      (T.iGate1 == this || T.iGate2 == this) {}                       // Already set
         ...
         {say(this);
          if (t.pin == null)
            stop("Gate:", T.name, "driven by too many other gates, including one from gate:", name);
          else
            stop("Gate:", T.name, "does not have enough pins to be driven by:", t, "from", name);
         }
       }
     }
...
//FIX need better estimate of size
    ...
    for (Gate g : gates.values())
     {if (g.iGate1 != null)                                                     // Input 1
       {final Gate s = g.iGate1;
      ...
      if (g.iGate2 != null)                                                     // Input2
       {final Gate s = g.iGate2;
