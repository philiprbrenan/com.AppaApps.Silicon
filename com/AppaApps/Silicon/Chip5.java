...
// Evaluate output pin order forwards and in reverse and choose the order with the shortest wiring length
import java.io.*;
import java.util.*;
...
public class Chip                                                               // Describe a chip and emulate its operation.
 {final static int maxSimulationSteps = 100;                                    // Maximum simulation steps
  ...
  final static int     layersPerLevel =   4;                                    // Ther sre 4 layers in each layer: insulation, x cross bars, x-y connectors and insulation, y cross bars
  ...
  int             mostCountedDistance;                                          // The number of gates at the distance from the outputs that has the most gates
  ...
  public String toString()                                                      // Convert chip to string
   {final StringBuilder b = new StringBuilder();
    b.append("Chip: "+name+"  Maximum distance: " + maximumDistanceToOutput);
    b.append("  mostCountedDistance: "            + mostCountedDistance);
    b.append("  countAtMostCountedDistance: "     + countAtMostCountedDistance);
    b.append("\n");
    b.append("Name      Operator  #  11111111  22222222  Input111-P=#  Input222-P=#  C Frst Last  Dist   Nearest  Px__,Py__  Drives these gates\n");
  ...
  class Gate                                                                    // Description of a gate
   {final int               seq = nextGateNumber();                             // Sequence number for this gate
    ...
    TreeSet<String>     outputs = new TreeSet<>();                              // The names of the gates that are driven by the output of this gate
    ...
    public String toString()                                                    // Convert to string
     {final char v = value == null ? '.' : value ? '1' : '0';
      ...
      if (op == Operator.Input)
        return String.format("%8s  %8s  %c"+" ".repeat(79)+"%4d,%4d\n",
          name, op, v, px, py);

       final Boolean pin1 = input1 != null ? whichPinDrivesPin1() : null;
       final Boolean pin2 = input2 != null ? whichPinDrivesPin2() : null;

      return   String.format("%8s  %8s  %c  %8s  %8s  %8s-%c=%c  %8s-%c=%c  %c %4d %4d  %4d  %8s  %4d,%4d  ",
        name, op, v,
        input1 == null ? ""  : input1,
        input2 == null ? ""  : input2,
    ...
    boolean whichPinDrivesPin(String driving)                                   // True if the driving gate is using pin 1 to drive this gate, else false meaning it is using pin 2
     {final Gate d = gates.get(driving);                                        // Driving gate
      if (d == null) stop("Invalid gate name:", driving);                       // No driving gate
      if (d.outputs.size() == 0) {say("Gate name:", driving, "is referenced as driving gate:", name, "but does not drive it"); return false;}
      return name.equals(d.outputs.first());                                    // First/Second output pin drives first input pin
     }
    ...
    boolean whichPinDrivesPin1()                                                // True if output pin 1 drives input pin 1, false if output pin 2 drives pin 1, else null if not known
     {return whichPinDrivesPin(input1);                                         // Gate driving pin 1
     }
    ...
    boolean whichPinDrivesPin2()                                                // True if output pin 1 drives input pin 2, false if output pin 2 drives pin 2, else null if not known
     {return whichPinDrivesPin(input2);                                         // Gate driving pin 2
     }

    void compileGate()                                                          // Locate inputs for gate so we do not have to look them up every time
     {if (input1 != null && !gates.containsKey(input1))                         // Input 1
        stop("No such input", input1, "for gate", name);
      if (input2 != null && !gates.containsKey(input2))                         // Input 2
        stop("No such input", input1, "for gate", name);
      if (input1 != null) iGate1 = gates.get(input1);                           // Address gate driving first input
      if (input2 != null) iGate2 = gates.get(input2);                           // Address gate driving second gate
      if (iGate1 != null) iGate1.outputs.add(name);                             // Update source with target
      if (iGate2 != null) iGate2.outputs.add(name);                             // Update source with target
     }
...
say("BBBB", name, op, N);
      ...
      final String[]G = outputs.toArray(new String[outputs.size()]);
      ...
      if (N % 2 == 1)                                                           // Odd number of gates driven
       {final Gate g = new Gate(Operator.FanOut);                               // The new fan out
        ...
        for(int i = 0; i < N-1; ++i)
         {final Gate t = gates.get(G[i]);
          if (t.input1 == name) t.input1 = g.name; else t.input2 = g.name;
          g.outputs.add (t.name);                                               // Put the target to the newly created gate
          outputs.remove(t.name);                                               // Remove the target from the original gate
         }
        outputs.add(g.name);                                                   // The old gate drives the new gate
      ...
      for(int i = 0; i < N/2; ++i)                                              // Lower half
       {final Gate t = gates.get(G[i]);
        ...
        g.outputs.add(t.name);
        outputs.remove(t.name);
       }
      outputs.add(g.name);                                                      // The old gate drives the new gate
      ...
      for(int i = N/2; i < N; ++i)                                              // Upper half
       {final Gate t = gates.get(G[i]);
        ...
        f.outputs.add(t.name);
        outputs.remove(t.name);
       }
      outputs.add(f.name);                                                      // The old gate drives the new gate
   ...
   {outputGates.clear();
    for(Gate g : gates.values())                                                // Each gate
     {g.distanceToOutput = null;                                                // Reset distance
      if (g.op == Operator.Output)                                              // Locate outputs
       {outputGates.add(g.name);
        g.nearestOutput = g.name;
       }
     }

    TreeSet<String> check = outputGates;                                        // Search from the outputs
    for (int d = 0; d < gates.size() && check.size() > 0; ++d)                  // Set distance to nearest output
     {TreeSet<String> next  = new TreeSet<>();
      ...
      final TreeSet<String> to = new TreeSet<>();                               // Gates at this distance from outputs
       ...
       {final Gate g = gates.get(name);
        if (g.distanceToOutput == null)                                         // New gate
         {to.add(name);                                                         // Classify gate by distance from gate
          ...
          final String[]n = {g.input1, g.input2};
          for(String h : n)                                                     // Next gates to search
           {if (h != null)
              {next.add(h);
               gates.get(h).nearestOutput = g.nearestOutput;
              }
            }
         }
       }
    ...
    mostCountedDistance   = 0;                                                  // The number of gates at the distance from the outputs that has the most gates
  ...
  void compileChip()                                                            // Check that an input value has been provided for every input pin on the chip.
   {final Gate[]G = gates.values().toArray(new Gate[0]);
    for(Gate g : G) g.outputs.clear();                                          // Clear forward references to pins that this gate drives
    for(Gate g : G) g.fanOut();                                                 // Fan the output of this gate if necessary
    for(Gate g : G) g.compileGate();                                            // Each gate on chip
    for(Gate g : G) g.fanOut();                                                 // Fan the output of this gate if necessary
   ...
   {final int B = sizeBits(output);                                             // Number of bits in bus
    ...
    for (int i = 1; i <= B; i++)                                                // Each bit on bus
     {final String n = n(i, output);                                            // Name of gate supporting named bit
      final Gate g = gates.get(n);                                              // Gate providing bit
...
//D1 Layout                                                                     // Layout the gates and connect them with wires
   ...
   {final Stack<String> r = new Stack<>();                                      // Order of gates
    ...
    for(String G : columns.get(Distance-1))
     {final TreeSet<String> p = new TreeSet<>();                              // Gates nearest
      order.put(G, p);
      final Gate g = gates.get(G);                                            // Gate details
  ...
  void layout(int Gsx, int Gsy)                                                 // Layout with gates scaled by gsx and gsy.  Normally gates are 2 by 2 in size, unless globally magnified by these factors.
   {gsx = Gsx; gsy = Gsy;
    if (layoutsDrawn.contains(name)) return;                                    // Only draw each named chip once
    layoutsDrawn.add(name);                                                     // Show that we have drawn this chip already

    final int sx =  2 * gsx, sy = 2 * gsy;                                      // Size of gate
    layoutX = sx * (sx + maximumDistanceToOutput);                               // X dimension of chip
    layoutY = sy * (sy + countAtMostCountedDistance);                            // Y dimension of chip
    ...
    for   (Integer D : distanceToOutput.keySet())                               // Gates at each distance from the outputs
     {final TreeSet<String> d = distanceToOutput.get(D);                        // Gates in this column
      ...
      for (String name : sortIntoOutputGateOrder(D))                            // Gates at each distance from the outputs
       {final Gate g = gates.get(name);
    ...
    diagram = new Diagram(layoutX, layoutY, gsx, gsy);                          // Layout the chip

    for (Gate g : gates.values())
     {final String i1 = g.input1, i2 = g.input2;
      if (i1 != null)                                                           // Input 1
       {final Gate s = gates.get(i1);
        final boolean f = g.whichPinDrivesPin1();                               // First/Second output pin drives first input pin
        diagram.new Wire(s.px, s.py+(f ? 0 : gsy), g.px+gsx, g.py);
       }
      if (i2 != null)                                                           // Input2
       {final Gate s = gates.get(i2);
        final boolean f = g.whichPinDrivesPin2();                               // First/Second output pin drives second input pin
        diagram.new Wire(s.px, s.py+(f ? 0 : gsy), g.px+gsx, g.py+gsy);
       }
     }
    diagram.gds2();
   }
      ...
      void removeFromCrossBars(Level level)                                     // Remove pixel from crossbars. The metal around the via has already been removed
       {final int w = width  != null ? width  : 0;
        ...
        for   (int x = 0; x <= w; ++x)                                          // We have to go one beyond the end of the segment to ensure that we do not inadvertently combine with a neighboring segment in the same cross bar.
         {for (int y = 0; y <= h; ++y)
           {if      (w > 1)
             {level.ix[c.x+x][c.y] = F;
             }
            else if (h > 1)
             {level.iy[c.x][c.y+y] = F;
             }
...
say("AAAA"); say(c);
  ...
  public static void main(String[] args)                                        // Test if called as a program
   {//test_and();
    ...
    test_chooseWordUnderMask();  //FF

    gds2Finish();                                                               // Collect resulting perl code to create GDS2 files
