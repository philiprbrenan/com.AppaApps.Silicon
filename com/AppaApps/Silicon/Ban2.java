...
//  Bits    Result = C.orBits("result", r1, r2);                                // Combine results of all immediate operations
  ...
  static void test_decode_RV32I ()                                              // Decode an immediate instruction
   {final int    N = 4;                                                         // Register widths in bits
    ...
    Chip         C = new Chip();                                                // Create a new chip
    Pulse    start = C.pulse("start",  0, N);                                   // Start pulse
    Pulse   update = C.pulse("update", 0, N, 32);                               // Update registers at end of decode/execute
    Bits       one = C.bits ("one",    N, 1);                                   // Constant one
    Bits      insw = C.bits ("insw",   N, InstructionWidthBytes);               // Instruction width

    Bits        x0 = C.bits ("X0",     N, 0);                                   // Define registers and zero them at the start
    Bits    nextPC = C.new BitBus("nextPC", N);                                 // Next value for program counter forward declaration
    Bits    result = C.new BitBus("result", N);                                 // Results of integer operation forward declaration

    Register    pc = C.register("pc", C.regIn(x0, start), C.regIn(nextPC, update)); // Risc V registers
    Register    x1 = C.register("x1", new RegIn(x0, start), new RegIn(result, C.pulse(n(1, "tp"))));
    Register    x2 = C.register("x2", new RegIn(x0, start), new RegIn(result, C.pulse(n(2, "tp"))));
    Register    x3 = C.register("x3", new RegIn(x0, start), new RegIn(result, C.pulse(n(3, "tp"))));
    ...
    Bits          PC = C.chooseFromTwoWords("PC", pc4, branch, branchInstr);    // Advance normally or jump by branch instruction immediate amount
    ...
    Words    targets = C.words      ("targets", N, 1, 2, 3);                    // Target register to load result into
    Pulse        ltd = C.delay      ("ltd",  start, 32);                        // 32 Wait this long before attempting to load the targets. Its crucial that this be just long enough for all the computation to complete otherwise we get a partial answer which is usually wrong. Bit on the other hand the smaller this number the better.
    Pulse[]       tp = C.choosePulse("tp", targets, rd, ltd);                   // Pulse loads target register
    ...
    f3_bgeu.anneal();
      ...
      ltu12.anneal();
        ...
        pc4.anneal();
    C.executionTrack(
      "pc    pci   pc4   PC    ltd start e   l",
      "%s  %s  %s  %s  %s   %s     %s %s",
       pc, pci, pc4, nextPC, ltd, start, C.collectBits("pc_e", N), C.getGate("pc_l"));
