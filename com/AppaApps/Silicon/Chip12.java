 ...
 {final static boolean github_actions =                                         // Whether we are on a github
  ...
  enum Operator                                                                 // Gate operations
   {And, Continue, FanOut, Forward, Gt, Input, Lt, My,                          // The forward gate is used to create a forward gate definition that will be replaced later in the design with an actual gate definition
    ...
    One, Or, Output, Pulse, Xor, Xxx, Zero;
   }
  ...
  boolean monad(Operator op)                                                    // Whether the gate takes a single input or not
   {return op == Operator.Continue || op == Operator.Not ||
           op == Operator.Output   || op == Operator.Pulse;
   }
  ...
  class Gate extends Bit                                                        // Description of a gate that produces a bit
   {final Operator     op;                                                      // Operation performed by gate
    ...
    void updateEdge()                                                           // Update a memory gate on a falling edge. The memory bit on pin 2 is loaded when the value on pin 1 goes from high to low because reasoning about trailing edges seems to be easier than reasoning about leading edges
     ...
     else if (op == Operator.Pulse)                                             // Trigger a unit pulse if we have received a falling edge
       {if (iGate1.value != null && iGate1.value && iGate1.nextValue != null && !iGate1.nextValue) // Falling edge on pin 1
         {pulsed = true;
         }
       }
     ...
     {final Boolean g = iGate1 != null ? iGate1.value : null,
      ...
      nextValue = switch(op)                                                    // Null means we do not know what the value is.  In some cases involving dyadic commutative gates we only need to know one input to be able to deduce the output.  However, if the gate ouput cannot be computed then its result must be null meaning "could be true or false".
       {case And     ->{if (g != null && G != null)  yield   g &&  G;  else if ((g != null && !g) || (G != null && !G)) yield false; else yield null;}
        ...
        case Pulse   -> {if (pulsed) {pulsed = false; yield true;} else yield false;}
  ...
  Gate Pulse   (CharSequence n, Bit...i)      {return FanIn(Operator.Pulse,    n, i);}
...
// Simulation                                                                   // Simulate the behavior of the chip
  ...
  Diagram simulate(Inputs inputs)                                               // Simulate the operation of a chip
   {compileChip();                                                              // Check that the inputs to each gate are defined
    ...
    //for (Pulse      p : P) p.update();                                        // Update edge of external pulses if pulses
...
//D2 Pulses                                                                     // Timing signals

 class Pulse extends Gate                                                       // A periodic pulse that drives an input bit.  The driving either starts at the beginning of the simulation or on the falling edge of a specified bit.
   {final Bit  driver;                                                          // Optionally a bit whose falling edge triggers the pulse
    Gate driveGate;                                                             // Gate associated with driving bit
    int lastStep = 1;                                                           // Last step at which the pulse started
    ...
    final int   start;                                                          // Number of cycles to wait before starting this pulse. Only applies to external pulses.
    Pulse(String Name, Bit Driver, int Period, int On, int Delay, int Start)    // Pulse definition as an input gate. Start n means start after n periods.
     {super(Operator.Input, Name, null, null);                                  // Pulse name
      ...
      if (Driver != null && Period > 0) stop("Choose one of Driver!=null or Period!= 0");
      driver = Driver; period = Period; on = On; delay = Delay; start = Start;  // Pulse timing details
    ...
    void compile()                                                              // Get gate associated with bit
     {if (driver == null) return;
      driveGate = getGate(this);
     }
    void setState()                                                             // Set gate implementing pulse to current state.
     {if (driveGate != null && driveGate.fell) lastStep = steps;                // Update start of internally driven pulse
      if (steps >= lastStep + start*period)                                     // Has the pulse started
       {final int s = steps - lastStep;                                         // Steps into pulse
  ...
  Pulse pulse(String Name, int Period, int On, int Delay, int Start)            // Create an external pulse
   {return new Pulse(Name, null, Period, On, Delay, Start);
   }

  Pulse pulse(String Name, int Period, int On, int Delay)                       // Create an external pulse that starts immediately
   {return new Pulse(Name, null, Period, On, Delay, 0);
   }

  Pulse pulse(String Name, int Period, int On)                                  // Create an external  pulse with no delay
   {return new Pulse(Name, null, Period, On, 0, 0);
   }

  Pulse pulse(String Name, int Period)                                          // Create an external  pulse with no delay
   {return new Pulse(Name, null, Period, 1, 0, 0);
   }

  Pulse pulse(String Name, Bit Bit, int Delay, int On)                          // Create an internal pulse
   {return new Pulse(Name, Bit, 0, On, Delay, 0);
   }

  Pulse pulse(String Name, Bit Bit, int Delay)                                  // Create an internal pulse
   {return new Pulse(Name, Bit, 0, 1, Delay, 0);
   }
...
//   {return new Pulse(Output,trigger, 0, width, 0, 0);                           // Pulse definition as an input gate. Start n means start after n periods.
//   }
...
//D3 Delay                                                                      // Send a pulse one way or another depending on a bit allowing us to execute one branch of an if statement or the other and receive a pulse notifying us when the execution of the different length paths are complete.
  ...
  Bit delay(String Output, Bit input, int delay)                                // Create a delay chain so that one leading edge can trigger another later on as work is performed
   {Bit p = input;
    final int d = max(delay / 2, 1);                                            // Start of chain
    for (int i = 1; i <= d; i++)                                                // All along the delay line
      p = Pulse(i < d ? n(i, Output) : Output, p);                              // Connect the delay line
    return p;
   }
/*
...
""");
  ...
  static void test_delay()
   {Chip  c = new Chip();
    Pulse p = c.pulse ("start", 8, 2);
    Bit   q = c.pulse ("delay", p, 4);
    q.anneal();

    c.executionTrack("p d", "%s %s", p, q);
    c.simulationSteps(32);
say(c);
    c.simulate();
    c.printExecutionTrace(); stop();
...
// 6   1 0 1   1
  ...
  static void test_divide_clock()
   {Chip   c = new Chip();
    Pulse  i = c.pulse("i", 0, 2);
    Pulse  p = c.pulse("p", 2, 1);
    Bit    m = c.new Bit("m");
    Bit    o = c.Or("or", i, m);
    Bit    M = c.My("M",  p, o);
    Bit    N = c.Not(m,   M);
    ...
    c.executionTrack("i  p  m  o", "%s  %s  %s  %s", i, p, c.getGate(m), c.getGate(o));
    c.simulate();
    //c.printExecutionTrace();
    c.ok("""
Step  i  p  m  o
...
/*
  Pulse clockDivider(String output, Pulse clock, Pulse reset)                     // Divide a clock by two
   {Pulse  i = pulse  (n(output, "initialize"), 0, 2);                          // Initialize
    Bit    R = Not    (n(output, "reset"), reset);                              // Reset when zero
    Bit    m = new Bit(n(output, "m"));                                         // Forward declare
    Bit    d = Or(  output, i, m);                                              // Divided output
    Bit    M = My(n(output, "M"),  clock, d);                                   // Flip flop on each clock falling edge
    Bit    N = Nand(m, M, R);                                                   // Negative feed back coupled with with reset
    return new Pulse(d);                                                        // Divided clock
   }

  static void test_divide_clock_reset()
   {Chip   c = new Chip();
    Pulse  p = c.pulse("p", 2, 1);                                              // Clock
    Pulse  r = c.pulse("r", 0, 2, 8);                                           // Reset when zero
    Pulse  d = c.clockDivider("divide1", p, r);
    Pulse  D = c.clockDivider("divide2", d, r);
    c.layoutLTGates(0);                                                         // This circuit fails to draw!
    c.simulationSteps(24);
    c.executionTrack("p  r  d  D", "%s  %s  %s  %s", p, r, d, D);
    c.simulate();
//  c.printExecutionTrace();  stop();
    c.ok("""
Step  p  r  d
...
*/
  ...
  static void test_pulsed()
   {Chip  c = new Chip();
    Pulse p = c.pulse ("p", 0, 2);
    Bit   q = c.Pulse ("q", p);
    Bit   r = c.Pulse ("r", q);
    Bit   s = c.Pulse ("s", r);
    Bit   t = c.Pulse ("t", s);
    Bit   u = c.Pulse ("u", t);
    u.anneal();
    c.executionTrack("p q r s t u", "%s %s %s %s %s %s", p, q, r, s, t, u);
    c.simulationSteps(14);
    c.simulate();
    //c.printExecutionTrace(); stop();
