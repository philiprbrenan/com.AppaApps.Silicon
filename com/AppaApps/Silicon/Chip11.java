...
if (name.equals("start")) say("AAA1", steps, value, nextValue, fell);
     }
    ...
    void updateValue()                                                          // Update the value of the gate
     {if (op != Operator.My)
       {changed = !systemGate && nextValue != value;                            // Suppress changes made by the system clock otherwise a chip with a clock will never stabilize
        value   = nextValue;
if (name.equals("start")) say("AAA2", steps, value, nextValue, fell);
       }
     }
   ...
   {compileChip();                                                              // Check that the inputs to each gate are defined
    ...
    for (steps = 1; steps <= actualMaxSimulationSteps; ++steps)                 // Steps in time
     {loadClock();                                                              // Load the value of the clock into the clock input bus
      ...
      loadPulses();                                                             // A pulse is a gate and so we must set its next value  just like any other gate
 ...
 class Pulse extends Gate                                                       // A periodic pulse that drives an input bit.  The driving either starts at the beginning of the simulation or on the falling edge of a specified bit.
   {final Bit  driver;                                                          // Optionally a bit whose falling edge triggers the pulse
    ...
    void compile()                                                              // Get gate associated with bit
     {if (driver != null){driveGate = getGate(this); driveGate.setSystemGate();}// The input gate associated with this pulse. The gate will be driven by the simulator.
     }
...
if (name.equals("delay")) say("BBBB", steps, driveGate.fell?1:0, delay,'-',on, 'i',  i, lastStep, name, nextValue);
       }
